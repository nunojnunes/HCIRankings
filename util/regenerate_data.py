...
def process_article(elem, conf_filter: str, include_all: bool) -> Optional[Dict]:
    ...
    # Special handling for PACMHCI (Proc. ACM Hum. Comput. Interact.)
    # Maps to conference-specific area based on issue number (CSCW, ISS, etc.)
    if areaname == Area("pacmhci"):
        mapped_area = PACMHCI_NUMBER_TO_AREA.get(number)
        if mapped_area:
            areaname = mapped_area
        else:
            return None  # skip conferences we don't track (GROUP, CHI PLAY, etc.)
