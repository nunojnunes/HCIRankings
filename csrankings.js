...
(function (CSRankings) {
    ...
    CSRankings.systemsAreas = ["ubi_mobile", "immersive_vis", "intelligent", "accessibility"];
    CSRankings.theoryAreas = ["journals"];
    CSRankings.interdisciplinaryAreas = [];
        ...
        if (numAreas > 0) {
            ...
            s = `<div class="no-areas-selected">
                ...
                <div class="no-areas-pills">
                    <span class="area-pill-btn ai-pill" onclick="CSRankings.App.getInstance().activateAI()">Core HCI</span>
                    <span class="area-pill-btn systems-pill" onclick="CSRankings.App.getInstance().activateSystems()">Specialized</span>
                    <span class="area-pill-btn theory-pill" onclick="CSRankings.App.getInstance().activateTheory()">Journals</span>
                </div>
...
(function (CSRankings) {
    ...
    function setAllCheckboxes(fields, value, invalidateCache) {
        for (let i = 0; i < CSRankings.areas.length; i++) {
            ...
            if (!element)
                continue;
            if (value) {
                // Turn on all venues including next tier.
        ...
        for (let i = 0; i < fieldIndices.length; i++) {
            ...
            if (element) {
                ...
                if (item in CSRankings.childMap) {
                    // It's a parent.
                    element.disabled = false;
                    // Activate / deactivate all children including next tier.
    ...
    function handleParentCheckboxClick(field, fieldElement, _invalidateCache) {
        ...
        if (field in CSRankings.childMap) {
            const children = CSRankings.childMap[field];
    ...
    function handleChildCheckboxClick(field, _invalidateCache) {
        ...
        CSRankings.childMap[parent].forEach((k) => {
            ...
            anyChecked |= val;
            // allChecked means all conferences (including next tier) are on.
            allChecked &= val;
        });
...
(function (CSRankings) {
    ...
    function handleNavigation(params, query, invalidateCheckboxCache) {
        ...
        if (foundAll) {
            // Set everything including next tier venues.
            for (const item in CSRankings.topLevelAreas) {
                const element = document.getElementById(item);
                if (element) {
                    element.checked = true;
                    element.disabled = false;
                    if (item in CSRankings.childMap) {
                        // Activate all children including next tier.
                        CSRankings.childMap[item].forEach((k) => {
                            const childElement = document.getElementById(k);
                            if (childElement) {
...
(function (CSRankings) {
    ...
    function createTourSteps() {
        return [
            ...
            {
                id: 'welcome',
                title: 'Welcome to HCIRankings',
                text: `
                    <p>Looking for a PhD or research position in HCI? You're in the right place.</p>
                    <p>This site helps you find <strong>research-active HCI faculty</strong> in your area of interest.</p>
                `,
                buttons: [
                    {
                        text: 'Skip',
...
(function (CSRankings) {
    ...
    const categoryAreas = {
        'ai': [
            { id: 'hci_core', label: 'Core HCI' },
            { id: 'ubi_mobile', label: 'Ubiquitous & Mobile' },
        ],
        'systems': [
            { id: 'accessibility', label: 'Accessibility' },
            { id: 'design', label: 'Design & Tangible' },
            { id: 'intelligent', label: 'Intelligent UI' },
        ],
        'theory': [
    ...
    function initAreaDropdowns() {
        ...
        ['ai', 'systems', 'theory'].forEach(updateIndicatorState);
        // Listen for changes to main checkboxes to sync dropdowns
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('click', () => {
                setTimeout(() => {
                    ['ai', 'systems', 'theory'].forEach(cat => {
