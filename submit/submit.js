/**
 * HCIRankings Faculty Submission Form
...
// Configuration
const GITHUB_REPO = 'nunojnunes/HCIRankings';
...
function validateNameForUpdateRemove() {
    ...
    if (exactMatch) {
        ...
        if (closeMatches.length > 0) {
            ...
            setFieldStatus('name', 'error', 'Name not found in HCIRankings. Use the dropdown to select an existing entry.');
        }
    }
}
/**
...
function validateInstitution() {
    ...
    if (exists) {
        ...
        setFieldStatus('institution', 'error', 'Institution not in HCIRankings. <a href="https://github.com/nunojnunes/HCIRankings/issues/new" target="_blank">Request it first</a>.');
    }
    updatePreview();
}
/**
...
function createAddIssueUrl(data, notes, existingOldEntry) {
    ...
    const title = isReinstatement
        ? `[HCIRankings form submission] Reinstate ${data.name} (${data.institution})`
        : `[HCIRankings form submission] Add ${data.name} (${data.institution})`;
...
function createUpdateIssueUrl(oldEntry, newEntry, notes) {
    ...
    const title = isReinstatement
        ? `[HCIRankings form submission] Reinstate ${newEntry.name} (${newEntry.institution})`
        : `[HCIRankings form submission] Update ${newEntry.name}`;
    const changes = [];
...
function createRemoveIssueUrl(entry, reason, notes) {
    const title = `[HCIRankings form submission] Remove ${entry.name} (${entry.institution})`;
    const body = `### Action
...
function createBatchIssueUrl(entries, institution, notes) {
    ...
    const title = hasReinstatements
        ? `[HCIRankings form submission] Add/Reinstate ${entries.length} faculty from ${institution}`
        : `[HCIRankings form submission] Add ${entries.length} faculty from ${institution}`;
...
---
*Submitted via HCIRankings batch submission form*`;
    const params = new URLSearchParams({
        title: title,
